...
- [x] Living document model for `core/` established: seven template files in
  ...
  *(2026-09-18: rule files — every `CLAUDE.md` and `_meta/schema.md` — moved out
  of the skeleton; they live only in brain/ and reach sessions through a hook.
  See parking lot: "brain/ rules — single source".)*
...
| ~~**brain/ rules — single source**~~ | ~~Step 0~~ | **RESOLVED 2026-09-18.** Rule files were kept in two places (`.claude/brain-skeleton/` and brain/) and had drifted: 17 of them differed or existed on one side only. Root cause: Claude Code does not load `CLAUDE.md` from brain/ (a sibling directory, not part of the project tree — verified: nothing loads without `--add-dir` + `CLAUDE_CODE_ADDITIONAL_DIRECTORIES_CLAUDE_MD=1`, and nested folder rules never load; an `@` import of a path outside the project does not load in headless `claude -p`), so skills pointed at the skeleton copy instead. Fix: brain/ is the single source (it describes itself, also for other tools); the skeleton keeps only data templates; `.claude/hooks/brain-rules/inject.py` injects brain/'s root rules at `SessionStart` and each folder's rules on the first Read/Edit/Glob/Grep there (Write: `PreToolUse`, whose context arrives together with the write result, not before it). Access via Bash does not trigger it. How it works, limits, and what to do when adding folders or debugging: `.claude/hooks/brain-rules/README.md`. |
| **brain/ rules — scaffolding a new brain/** | when needed | **DEFERRED 2026-09-18.** Since rules live only in brain/, `/init-brain` creation mode builds a bare structure without any `CLAUDE.md`. Harmless with one brain/ (a new machine clones the brain/ repo), but a newcomer starting from this public repo gets no rules. Design worked out and then rolled back as premature: (1) `.claude/scripts/brain-rules-export.sh` copies every brain/ `CLAUDE.md` + `_meta/schema.md` into `.claude/brain-skeleton/` as a **generated snapshot**, never hand-edited and never read by skills, and removes rule files that disappeared from brain/. Idempotent. (2) Privacy guard: the skeleton is public, so the script aborts before copying if any regex from a gitignored `.claude/brain-rules-export.deny` matches. Force `LC_ALL=C.UTF-8`, otherwise Polish characters in patterns silently fail to match (`krak[oó]w` missed `Kraków`). (3) Before the first export, genericise personal examples **at the source in brain/**. As of 2026-09-18 these are: `core/contacts/` (a real partner's full name as the lookup example), `knowledge/events/` (companions' first names, real events, city), `knowledge/places/` (a real venue, street, city), `knowledge/finance/` and `finance/analyses/` (tickers that may reveal holdings). Re-scan all rule files, not just these. (4) `/init-brain`: creation mode copies the snapshot; validation mode skips rule files, never restoring them into an existing brain/ from the snapshot. (5) Review the skeleton diff before every commit. |
