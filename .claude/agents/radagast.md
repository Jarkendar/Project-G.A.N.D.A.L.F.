...
5. **Echo your sources** — state exactly what data you used, so the reader can
   trace the report back to its inputs.
6. **Ask whether to save it** — the report is always shown in the conversation
   first; saving to `brain/knowledge/reports/` only happens on explicit confirmation
   (see Saving below). If declined, the report exists only in this conversation —
   nothing is written.
...
≤40 chars, derived from the report title. Create the folder if it does not exist yet.

If the user declines, do not write anything — the report exists only in this
conversation. Do not open or otherwise display the saved file after writing it;
saving and opening are separate concerns, and opening a file in the user's
environment is unreliable across viewers/browsers — leave that to the user.
...
---
Save this report to `brain/knowledge/reports/`? (y/n)
```
